...
- Valid `/cadastru` search submissions are logged to `cadastru_search_events` for admin stats with `search_type`, optional authenticated `user_id`, optional derived district for address searches, cadastral number when known, result type, and timestamp. Result type is one of `no_data`, `address_only`, `apartment_only`, or `full_data`.
