...
- Authenticated users can see their own cadastru search rows in `/profile` history, where the result column shows the cadastral number and details show the logged analytics fields.
...
- Long-lived cadastru storage is written only when `NODE_ENV=production`. `cadastru_records.lookup_count` counts successful cadastral-number detail lookups.
