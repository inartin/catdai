...
- Favorites are persisted only when `NODE_ENV=production`; development responses do not write `user_favorites`.
...
- Short shared links are persisted only when `NODE_ENV=production`; development returns a direct `/evaluare` URL instead of creating a `shared_links` row.
