...
## Backend
- `POST /api/feedback` requires a valid Supabase bearer token.
...
## Schema
- `db/user_feedback.sql` creates `user_feedback` with `user_id`, `message`, optional image metadata/data, status, timestamps, indexes, and RLS policies.
