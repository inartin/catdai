...
Cadastral lookup is preview-gated with `cadastru_lookup` credits. The standalone `/cadastru` page accepts anonymous searches and returns the same server-masked result preview: direct cadastral-number searches keep the submitted number visible, address searches replace the discovered cadastral number with a fake blurred number, and address, floor, classifier, and construction year remain visible when available. Anonymous blurred values open the login popup; authenticated users without a remaining credit see the package purchase popup. The remaining official cadastru fields are server-masked with `|` characters and blurred in the UI.
